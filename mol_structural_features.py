...
def _calculate_atom_features_tensor(rdkit_mol: Chem.Mol, selected_features: List[str],
                                    molecule_index: Optional[int] = None, smiles: Optional[str] = None) -> torch.Tensor:
    """
    ...
    Args:
        ...
        smiles (Optional[str]): The SMILES string of the molecule, for error context. Defaults to None.
...
def _calculate_bond_features_tensor(rdkit_mol: Chem.Mol, pyg_edge_index: torch.Tensor, selected_features: List[str],
    ...
    if selected_features: # Only attempt if features are actually requested
        try:
            ...
            if single_bond_feature_length == 0:
                 logger.warning(f"[{molecule_index}, '{smiles}'] No valid bond features selected or they have 0 length. Defaulting to 1 for dummy feature length.")
    ...
    for i in range(num_edges):
        ...
        if features is None:
            logger.warning(f"[{molecule_index}, '{smiles}'] No RDKit bond found for PyG edge ({u}, {v}). Assigning zeros to features.")
...
def add_structural_features(
    ...
    Args:
        ...
        molecule_index (Optional[int]): The index of the molecule in the dataset. Used for detailed error reporting.
                                        Defaults to None.
        smiles (Optional[str]): The SMILES string of the molecule. Used for detailed error reporting.
                                Defaults to None.
    ...
    """
    log_prefix = f"[Mol Index: {molecule_index}, SMILES: '{smiles}']" if molecule_index is not None else ""
        ...
        if selected_atom_features:
            if rdkit_mol.GetNumAtoms() == 0:
                ...
                pyg_data.x = _calculate_atom_features_tensor(rdkit_mol, selected_atom_features,
                                                             molecule_index=molecule_index, smiles=smiles)
        else:
